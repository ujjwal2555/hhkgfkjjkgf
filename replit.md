...
### November 8, 2025 - UI/UX Enhancements & HR Features
- **Landing Page**: Created professional landing page with classy earth-tone aesthetic (amber/slate colors)
  - Hero section with WorkZen branding and call-to-action buttons
  - Features grid showcasing key HRMS capabilities
  - Responsive design following Material Design principles
  - Root path (/) now shows landing page; login moved to /login route
- **HR Employee Management**: Extended HR permissions to delete employees (previously Admin-only)
  - Backend: Updated DELETE /api/users/:id route to `requireRole('admin', 'hr')`
  - Frontend: Updated employee deletion UI to show delete button for both Admin and HR roles
- **Attendance Statistics**: Added comprehensive attendance tracking for individual employees (HR/Admin only)
  - New "Attendance" tab in employee profile showing detailed stats
  - Visual metrics cards displaying: Total Days, Present, Absent, Half-Day, Leave, and Attendance %
  - Recent attendance records table with date, clock in/out times, and status badges
  - Backend endpoint: GET /api/attendance/user/:id (role-gated to admin/hr)
  - Frontend: Role-based query with loading states and empty state handling
- **Role Management**: Added admin-only role change functionality
  - New "Change Role" button with shield icon in employee profile (admin-only visibility)
  - Interactive dialog with role selector (Employee, HR Officer, Payroll Officer, Administrator)
  - Visual display of permissions for each role to help admins make informed decisions
  - Backend security: Only admins can change roles; HR users receive 403 error if attempted
  - Frontend: Role change button, mutation handling, and cache invalidation
  - Proper permission checks prevent privilege escalation by non-admin users
